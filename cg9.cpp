...
public:
        ...
        // 値を出力する
        void print() { printf("Vector3d(%f %f %f)¥n", x, y, z); }
};
...
public:
        ...
        double getIntersec(Vector3d &p, Vector3d &v) {
                ...
                double t = -1;
                // ★ここで t の値を計算する
                // ★ただしz座標(奥行き)が-3000より小さいなら交わらないものとする
                //床のy座標は－１５０とわかっている
                //視点のp座標は引数で、もう上の方で定義されている、そのまま書けばいい
                t = (-150 -p.y) / v.y;
                if(p.z < -3000) return -1;
                return t;
        }
...
void getPixelColor(double x, double y, Vector3d &colorVec) {
        ...
        if(t_sphere > 0) { // 球との交点がある
            ...
            //cosa
                          //法線ベクトル
                          Vector3d koutenn = viewPosition + t_sphere * ray;
                          Vector3d hosenn = koutenn - sphere.center;
                          hosenn.normalize();
                          lightDirection.normalize();
                         //入射光法線

             //コサインを求める
                          double Cos = hosenn * (-lightDirection) ;
                          double r,g,b;
                                  ...
                                  if (Cos < 0){
                                          double Id = 0;
                                          double I = Is + Ia;
                                          r = std::min(I * sphere.cR, 1.0); // 1.0 を超えないようにする
                                          g = std::min(I * sphere.cG, 1.0); // 1.0 を超えないようにする
                                          b = std::min(I * sphere.cB, 1.0); // 1.0 を超えないようにする

                                          // 描画色の設定
                                          glColor3d(r, g, b);
                                  }else{
                                          double Id = Iin * Kd * Cos; // 拡散反射光
                                          double I = Id + Is + Ia;
                                           r = std::min(I * sphere.cR, 1.0); // 1.0 を超えないようにする
                                           g = std::min(I * sphere.cG, 1.0); // 1.0 を超えないようにする
                                           b = std::min(I * sphere.cB, 1.0); // 1.0 を超えないようにする
        ...
        if(t_board > 0) { // 床との交点がある
                ...
                double g = 1;
                double b = 1;
                if((x < 100 && x >= 0 && y < -100) || (x < -100 && y < -100) || (x > 100 && y > -100 && y < 0) || (x > -100 && x <= 0 && y < 0 && y > -100)){
                  r = 0.8;
                  g = 0.8;
                  b = 0.8;
                }
